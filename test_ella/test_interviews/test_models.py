...
class TestInterviewModels(InterviewTestCase):
    def test_has_replies_returns_false_when_no_replies(self):
        tools.assert_equals(False, self.interview.has_replies())

    def test_unanswered_questions_empty_with_no_questions(self):
        tools.assert_equals([], list(self.interview.unanswered_questions()))

    def test_unanswered_questions(self):
        q = Question.objects.create(interview=self.interview, content='What ?')
        tools.assert_equals([q], list(self.interview.unanswered_questions()))
...
class TestInterviewModel(TestCase):
    ...
    def test_ask_indicators_when_active(self):
        tools.assert_equals(True, self.interview.asking_started())
        tools.assert_equals(False, self.interview.asking_ended())
        tools.assert_equals(True, self.interview.can_ask())

    def test_ask_indicators_when_not_yet_active(self):
        now = datetime.now()
        day = timedelta(days=1)
        self.interview.ask_from = now+(day/2)
        tools.assert_equals(False, self.interview.asking_started())
        tools.assert_equals(False, self.interview.asking_ended())
        tools.assert_equals(False, self.interview.can_ask())
    ...
    def test_ask_indicators_when_already_inactive(self):
        ...
        tools.assert_equals(True, self.interview.asking_started())
        tools.assert_equals(True, self.interview.asking_ended())
        tools.assert_equals(False, self.interview.can_ask())

    def test_reply_indicators_when_active(self):
        tools.assert_equals(True, self.interview.replying_started())
        tools.assert_equals(False, self.interview.replying_ended())
        tools.assert_equals(True, self.interview.can_reply())

    def test_reply_indicators_when_not_yet_active(self):
        now = datetime.now()
        day = timedelta(days=1)
        self.interview.reply_from = now+(day/2)
        tools.assert_equals(False, self.interview.replying_started())
        tools.assert_equals(False, self.interview.replying_ended())
        tools.assert_equals(False, self.interview.can_reply())
    ...
    def test_reply_indicators_when_already_inactive(self):
        ...
        tools.assert_equals(True, self.interview.replying_started())
        tools.assert_equals(True, self.interview.replying_ended())
        tools.assert_equals(False, self.interview.can_reply())
