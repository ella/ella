...
def __mimeData(filename, msg):
    ...
    #TODO! Payload base64 encode
    data.set_payload(base64.encodestring(fp.read()))
    #data.add_header('Content-ID', '<%s>' % filename)
...
def create_mail(**kwargs):
    ...
    msgRoot.add_header('Precedence', 'Bulk')
    #msgRoot.add_header('Importance', 'Low')
    msgRoot.add_header('X-Mailer', 'Ella CMS')
    msgRoot['Subject'] = kwargs.get('subject', 'Newsletter')
