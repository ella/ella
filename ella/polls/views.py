...
def poll_check_vote(request, poll):
    ...
    """
    sess_jv = request.session.get(conf.POLL_JUST_VOTED_COOKIE_NAME, [])
    # removing just voted info from session
    if poll.id in sess_jv:
        del request.session[conf.POLL_JUST_VOTED_COOKIE_NAME]
        # TODO - del just my poll, not the entire list !
        return conf.USER_JUST_VOTED
    # removing no vote info from session
    sess_nv = request.session.get(conf.POLL_NO_CHOICE_COOKIE_NAME, [])
    if poll.id in sess_nv:
        del request.session[conf.POLL_NO_CHOICE_COOKIE_NAME]
        # TODO - del just my poll, not the entire list !
        return conf.USER_NO_CHOICE
    # authenticated user - check session
    if request.user.is_authenticated():
        sess = request.session.get(conf.POLL_COOKIE_NAME, [])
        if poll.id in sess:
            return conf.USER_ALLREADY_VOTED
        # otherwise check Vote object - just for sure
        if poll.check_vote_by_user(request.user):
            return conf.USER_ALLREADY_VOTED
        return conf.USER_NOT_YET_VOTED
    # anonymous - check cookie
    else:
        cook = request.COOKIES.get(conf.POLL_COOKIE_NAME, '').split(',')
...
def survey_check_vote(request, survey):
    sess_jv = request.session.get(conf.SURVEY_JUST_VOTED_COOKIE_NAME, [])
    # removing just voted info from session
    if survey.id in sess_jv:
        del request.session[conf.SURVEY_JUST_VOTED_COOKIE_NAME]
        # TODO - del just my poll, not the entire list !
        return conf.USER_JUST_VOTED
    # removing no vote info from session
    sess_nv = request.session.get(conf.SURVEY_NO_CHOICE_COOKIE_NAME, [])
    if survey.id in sess_nv:
        del request.session[conf.SURVEY_NO_CHOICE_COOKIE_NAME]
        # TODO - del just my poll, not the entire list !
        return conf.USER_NO_CHOICE
    # authenticated user - check session
    if request.user.is_authenticated():
        sess = request.session.get(conf.SURVEY_COOKIE_NAME, [])
    ...
    # anonymous - check cookie
        ...
        cook = request.COOKIES.get(conf.SURVEY_COOKIE_NAME, '').split(',')
...
def poll_vote(request, poll_id):
    ...
    # vote check
    if poll_check_vote(request, poll) != conf.USER_NOT_YET_VOTED:
        return HttpResponseRedirect(url)
    ...
    if not form.is_valid():
        ...
        request.session[conf.POLL_NO_CHOICE_COOKIE_NAME] = sess_nv
        return HttpResponseRedirect(url)
    ...
    request.session[conf.POLL_JUST_VOTED_COOKIE_NAME] = sess_jv
    ...
    if request.user.is_authenticated():
        sess = request.session.get(conf.POLL_COOKIE_NAME, [])
        sess.append(poll.id)
        request.session[conf.POLL_COOKIE_NAME] = sess
    # annonymous user vote - cookies update
    else:
        cook = request.COOKIES.get(conf.POLL_COOKIE_NAME, '').split(',')
        if len(cook) > conf.POLL_MAX_COOKIE_LENGTH:
            cook = cook[1:]
        cook.append(str(poll.id))
        expires = datetime.strftime(datetime.utcnow() + timedelta(seconds=conf.POLL_MAX_COOKIE_AGE), "%a, %d-%b-%Y %H:%M:%S GMT")
        response.set_cookie(
            conf.POLL_COOKIE_NAME,
            value=','.join(cook),
            max_age=conf.POLL_MAX_COOKIE_AGE,
            expires=expires,
            path='/',
...
def survey_vote(request, survey_id):
    ...
    # activity check
    if not survey.current_activity_state == conf.ACTIVITY_ACTIVE:
        return HttpResponseRedirect(url)

    # vote check
    if survey_check_vote(request, survey) != conf.USER_NOT_YET_VOTED:
        return HttpResponseRedirect(url)
    ...
    if not form.is_valid():
        ...
        request.session[conf.SURVEY_NO_CHOICE_COOKIE_NAME] = sess_nv
        return HttpResponseRedirect(url)
    ...
    request.session[conf.SURVEY_JUST_VOTED_COOKIE_NAME] = sess_jv
    ...
    if request.user.is_authenticated():
        ...
        request.session[conf.SURVEY_COOKIE_NAME] = sess
    # annonymous user vote - cookies update
    else:
        cook = request.COOKIES.get(conf.SURVEY_COOKIE_NAME, '').split(',')
        if len(cook) > conf.SURVEY_MAX_COOKIE_LENGTH:
            cook = cook[1:]
        cook.append(str(survey.id))
        expires = datetime.strftime(datetime.utcnow() + timedelta(seconds=conf.SURVEY_MAX_COOKIE_AGE), "%a, %d-%b-%Y %H:%M:%S GMT")
        response.set_cookie(
            conf.SURVEY_COOKIE_NAME,
            value=','.join(cook),
            max_age=conf.SURVEY_MAX_COOKIE_AGE,
            expires=expires,
            path='/',
            domain=Site.objects.get_current().domain,
            secure=None
...
def contest_vote(request, context):
    ...
    context.update({
            ...
            'activity_not_yet_active' : conf.ACTIVITY_NOT_YET_ACTIVE,
            'activity_active' : conf.ACTIVITY_ACTIVE,
            'activity_closed' : conf.ACTIVITY_CLOSED
        })
