...
def listener_post_save(sender, signal, created, **kwargs):
    print 'Listener activated by %s, sig=%s, created=%s' % (sender, signal, created)
    print 'Listener kwargs=%s' % kwargs
...
class RichTextField(fields.Field):
    ...
    def get_source_text(self):
        if not (self.instance and MARKUP_APP_INSTALLED):
            return
    ...
    def clean(self, value):
        ...
        if self.instance:
            ...
            default_proc = TextProcessor.objects.get(name=conf.NEWMAN_MARKUP_DEFAULT)
