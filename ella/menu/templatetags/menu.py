...
#TODO @cache_this
def render_common(context, menu, obj):
    mi = MenuItem.objects.get_for_menu(menu=menu)
    mih = highlight_items(context, mi, obj)
    context[MENU_TPL_VAR] = mih
...
class MenuForObjectNode(template.Node):
    ...
    def render(self, context):
        ...
        if not menus:
            log.warning('There are no Menu instances named "%s" for object %s' % (self.menu_slug, self.object))
            return ''
        return render_common(context, menus[0], self.object)

class MenuSimpleNode(template.Node):
    def __init__(self, menu_slug):
    ...
    def render(self, context):
        ...
        if not menus:
            ...
            return ''
        return render_common(context, menus[0], None)
...
def menu(parser, token):
    ...
    Syntax::
        ...
        {% menu MENU_NAME [ for (APPLICATION.MODEL with FIELD VALUE | OBJECT) ]  %}

    Example usage::

        {% menu horizontal_menu %}       # constructs horizontal menu
        {% menu mainmenu for category with slug "zena"  %}
        {% menu mainmenu for articles.article with pk 21 slug blabla field value %}
        {% menu mainmenu for articles.article with pk obj.pk  %}
        {% menu mainmenu for object  %}  # object is template variable containing model instance, i.e. Category instance
    """
